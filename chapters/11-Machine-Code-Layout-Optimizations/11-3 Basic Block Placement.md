...
// 热路径继续
```
图 @fig:BBLayout 显示了此代码片段的两种可能的物理布局。图 @fig:BB_default 是大多数编译器在没有提供提示时默认发出的布局。如果我们反转条件 `cond` 并将热代码作为直通放置，则可以实现图 @fig:BB_better 中显示的布局。
