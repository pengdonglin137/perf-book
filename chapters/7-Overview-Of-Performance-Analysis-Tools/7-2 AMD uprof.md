...
@fig:uProfHotspots 显示了*函数热点*分析（在图像左侧的菜单列表中选择）。在图像顶部，你可以看到一个事件时间线，显示在应用程序执行的各个时间观察到的事件数量。在右侧，你可以选择要绘制的指标；我们选择了 `RETIRED_BR_INST_MISP`。注意在 20s 到 40s 的时间范围内分支预测错误的尖峰。你可以选择此区域以密切分析那里发生了什么。一旦你这样做，它将更新底部面板以仅显示该时间间隔的统计信息。
